...
```
POST /docx/v1/documents/{docId}/blocks/{docId}/children
Body: { "children": [{ "block_type": 23, "file": { "token": "" } }], "index": -1 }
Response: { "data": { "children": [{ "block_id": "<文件块ID>" }] } }
```

注意：实际返回/文档结构中可能先出现 `block_type: 33` 的 View 包装块，真正的文件块是其子级 `block_type: 23`。后续上传的 `parent_node` 和更新块的 `block_id` 必须使用真正的 File Block ID。
...
Fields:
  file_name: "video.mp4"
  parent_type: "docx_file"    ← 必须是这个值
  parent_node: "<文件块ID>"    ← 文件块ID，不是文档ID！
  size: <文件字节数>
  file: <二进制文件内容>
Response: { "data": { "file_token": "xxx" } }
```
...
Body: { "file_name": "video.mp4", "parent_type": "docx_file", "parent_node": "<文件块ID>", "size": <文件字节数> }
...
Body: { "upload_id": "<upload_id>", "block_num": <分片数量> }
Response: { "data": { "file_token": "xxx" } }
```

### Step 3：绑定文件
```
PATCH /docx/v1/documents/{docId}/blocks/{文件块ID}
