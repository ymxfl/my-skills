 ...
 *   node check_feishu_auth.js --create-test-doc
 *
 * 从以下来源读取凭证（优先级从高到低）：
 *   1. 命令行参数 --app-id / --app-secret
 *   2. 环境变量 FEISHU_APP_ID / FEISHU_APP_SECRET
 *   3. .env 文件（skill 根目录、当前工作目录；后者优先）
 */
...
const SHOULD_CREATE_TEST_DOC = hasFlag('--create-test-doc');
...
【第五步】配置凭证
  ...
  可选：如需真实创建测试文档验证 docx 权限，请追加 --create-test-doc。
...
async function checkAuth() {
  ...
  console.log('   App Secret:', maskSecret(APP_SECRET));
  ...
  // 3. 可选：真实创建文档测试权限。默认跳过，避免留下需要手动删除的测试文档。
  if (SHOULD_CREATE_TEST_DOC) {
    try {
    ...
    console.log('ℹ️  已跳过创建测试文档；如需验证 docx 创建权限，请追加 --create-test-doc');
  }

  console.log('\n🎉 飞书凭证验证完成！');
  console.log('   可以开始运行 video_to_feishu.js 了\n');

  // 5. 输出脱敏后的当前配置，方便确认来源；不回显完整 secret。
  console.log('── 当前检测值（已脱敏）──');
  console.log(`FEISHU_APP_ID=${APP_ID}`);
  console.log(`FEISHU_APP_SECRET=${maskSecret(APP_SECRET)}`);
}
